...
renderChatRoom(chat) {
    ...
    parseSpecialMessage(content) {
        ...
        const transferSummaryMatch = trimmedContent.match(/^\[\s*转账\s*[¥￥]\s*([\d,]+(?:\.\d+)?)\s*元?\s*\]\s*(?:[（(]\s*状态\s*[：:]\s*(未收款|待收款|已收款|已退回)\s*[）)])?\s*$/);
