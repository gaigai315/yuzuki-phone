...
根据剧情，仅当其他角色给{{user}}发微信时，请在正文回复末尾使用<wechat>标签输出微信内容，并使用<!---->包裹。
注意：只有当剧情正文里确实有角色使用手机发给{{user}}发送消息是,才使用此标签，其他角色之间的消息请勿使用该标签及规则。
...
【特殊微信代发格式规则】
1. 触发条件：当且仅当剧情中出现“其他角色拿走或使用user的手机，以user的身份代替user给其他人发送微信消息”的情节时，必须触发此特定格式。若角色只是与user处于同一场景中操作自己的手机，或在user面前展示、编辑、转发信息，不适用此格式。
