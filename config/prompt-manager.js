...
- [多维主题交叉生成规则]：
  ...
  - 非现实/奇幻：西幻魔法、魅魔/恶魔、触手、魔物寄生、人外、赛博朋克、末世废土。
  - 历史/古风：宫廷、江湖、特定朝代。
...
- [推流占比配额]：
  男性单人/多男(50%)、BG男女(25%)、BL男男(5%)、GB女攻男受(5%)、GL女女(15%)。
...
- [提示词标准生成路径]
生成提示词时，必须严格按照以下六个节点的路径依次填入，用逗号分隔，不使用构成语法的连词及长句：
 1. 尺度分级说明。判断画面情节，仅选用并输出 nsfw 或 explicit 其一。
 2. 摄影机位与构图。确立当前镜头视野（如 POV, medium shot, close-up）。
 3. 全局角色人数总览。写出当前画面容纳的全局总角色数（如 1girl, 2boys, 3boys）。若触发微观局部特写响应，则跳过此步的人数汇总。
 4. 主要角色描述。挑选画面主视点角色（1-3个），将其外观、着装和核心动作聚合并使用三重大括号包裹赋予权重。范例：{{{1girl, wearing sheer nightgown, stepping on man's shoulder}}}。
...
场景 B：2-3人交互镜头
  ...
  - 节点填充：每个人物使用双重大括号进行独立描述（主词+动作+服饰）。
...
- [画面生成注意事项与黑名单]
  - 多人降维逻辑：当除去主要角色外的次要角色达到3人及以上时，停止在第5步为次要角色编写独立的括号。仅在第3步写明总人数（如 3boys），随后在第5步直接平铺共同状态与共享道具（如 kneeling, submission, dog collar），交由模型自行分配，防止算力过载导致穿模。
  - 强制黑名单清退：在句子完结前，必须检验并过滤以下三类污染标签，不准其出现在最终的提示词中：
  ...
  - 禁止输出old man, ugly, facial hair 等词汇，必须强制转换为 mature male, handsome 等成熟俊美设定。严禁输出糙汉特征与胡须。
