...
3.任务分别四个等级，仅有：[普通]、[中级]、[高级]、[特级]。随机生成不同等级的任务，且任务难度必须适配等级。
4.每个任务必须完整输出模板中的全部字段，不得省略字段或改变字段名。
5.发布时间仅显示时分秒；剩余时间使用“x天 hh:mm:ss”或“hh:mm:ss”；发布者可以为个人昵称或组织联系人。
6.任务内容应包含背景、风险和执行要求；任务目标生成2-4项，统一使用“目标名称｜0/总数量”格式，新任务的当前进度必须为0。
7.奖励和声望值必须只显示数字，无需带单位；额外奖励可以是物品名称、数量或“无”。
8.每个任务生成0-3条符合世界背景的任务讨论，严格使用“昵称｜Lv.xx｜时间｜讨论内容”格式；没有讨论时在“任务讨论:”后留空。
9.必须参考【当前手机时间】生成合理的发布时间与剩余时间，不得生成时间逻辑冲突的任务。`,
                    order: 1
                }
            },
