...
💡 使用本地专属表情包时，格式：发送者: [表情包](表情名称)；若清单里没有合适项，也可继续使用关键词搜索。`;
        ...
        if (!text.includes('窗口隔离：单聊模式下只允许输出当前窗口')) {
            text = `${text}
...
💡 群聊中若要使用本地表情包，格式为：发送者: [表情包](表情名称)。`;
        ...
        if (!text.includes('多窗口联动（仅群聊模式可选）')) {
            text = `${text}
💡 多窗口联动（仅群聊模式可选）：
1. 默认只输出当前群窗口 ---{{groupName}}---。
2. 仅当剧情明确出现“群成员转私聊{{user}}”时，才允许在同一个<wechat>标签内追加 1 个私聊窗口。
3. 私聊窗口名必须使用【{{user}}的微信好友列表】中的完整好友名，禁止使用{{user}}本人名字。
