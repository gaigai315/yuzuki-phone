...
【用户账号粉丝数量规则】
- 系统提供的“当前粉丝数量”是唯一基准，严禁根据用户人设、身份、职业或知名度重新估算、重置粉丝数。
- 默认保持当前值不变。只有本轮剧情中出现与用户微博账号直接相关、已经公开且明确会引发关注或取关的事件时，才允许调整；浏览热搜、刷新推荐、普通评论和与用户账号无关的剧情均不得改变粉丝数。
- 输出的是变化后的粉丝总数，不是新增/减少量。普通互动只能小幅波动；只有明确的爆款传播、公开出圈或重大负面舆情才允许较大波动，但严禁跨数量级跳变。
- AI 没有返回粉丝数、没有明确变化依据或无法判断时，必须维持当前值；禁止输出 0、负数、区间、约数、“未知”或空值。
- 文本显示规则：1000 至 9999 可使用 k，10000 及以上可使用 w；可保留一位小数，但换算后的数值必须与粉丝总数一致。JSON 的 followers 字段必须使用正整数。
