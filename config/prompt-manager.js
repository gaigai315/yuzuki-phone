...
排除条件：【 非发给{{user}}的通讯（如npc→char，npc→npc，char→char之间的消息】）严禁使用<wechat>标签输出只在正文中描写即可。严禁代替{{user}}使用此标签给其他人发消息；严禁当前正文中角色并未使用手机微信给{{user}}发消息时，凭空描写微信消息。
...
⑤内心活动：仅限其他角色发送给{{user}}的单聊普通文字消息。当发送者有强烈但未说出口的真实想法时，可在同一条消息末尾追加 [内心]（未说出口的想法）；必须与说出口的消息写在同一行，不得单独成条，不得用于{{user}}发送的消息、群聊消息或转账/红包/图片/语音条等特殊消息。
...
【严格遵循单聊/群聊格式：仅当剧情明确发生角色通过该群聊发送消息时，才使用群聊格式；否则一律使用单聊格式。】
<wechat><!--
---好友A---
接收人：{{user}}
date:{{STORY_DATE}}
[HH:MM] 好友A的消息1
[HH:MM] 好友A的消息2
[HH:MM] 没事，我很快就到。[内心]（其实我已经在门外站了很久。）
---群名---
接收人：{{user}}
type:group
date:{{STORY_DATE}}
...
[HH:MM] [拨打微信语音]
[HH:MM] [语音条]（语音条转文字内容）
[HH:MM] 😀（普通emoji必须直接输出Unicode符号，禁止写成[微笑]、[偷笑]、[加油]等文字标签；仅狗头这个特殊表情输出[狗头]）
...
【手机微信调用准则】：
[<wechat>手机消息标签]触发条件：仅当剧情正文中发生【涉及{{user}}的手机通讯】时才使用此标签。允许{{user}}使用手机微信并发送给角色（char/npc）的消息
...
绝对禁止使用的情况（一旦出现立刻停止输出标签）：
 ...
 任何没有打到{{user}}手机上的电话（如：NPC打给NPC，char打给NPC等）。`;
    ...
    _getDefaultUndercoverDealerPromptTemplate() {
        return [
    ...
    _getDefaultUndercoverSpeechPrompt() {
        return [
...
15. 红包/转账处理：当{{user}}向{{chatName}}发起微信转账或红包时，你必须以{{chatName}}的人设判断是否收下或退回；如果收下，必须单独输出 {{chatName}}: [收款]/{{chatName}}: [领取红包]；如果退回，必须单独输出 {{chatName}}: [退回转账] 或 {{chatName}}: [退回红包]。
...
{{chatName}}: 有什么事吗
{{chatName}}: 我到了，开门[转线下]
{{chatName}}: 😀（普通emoji必须直接输出Unicode符号，禁止写成[微笑]、[偷笑]、[加油]等文字标签；仅狗头这个特殊表情输出[狗头]）
{{chatName}}: [转账](金额：100元)
...
5. 好友名称后必须输出对应性别。根据剧情和人设精准判断好友的性别，禁止使用未知、不确定等描述。性别只能写在每行末尾，并严格使用“-男”或“-女”，例如“张三（同事）-男”，不要把性别写进备注括号。
...
**开始任务！**请在综合了解后，直接输出<初始化设定>中的内容，请勿回复剧情或其他无关内容。`,
    ...
    momentsInteraction: {
        ...
        content: `你是一个专业的小说角色/社交动态演绎专家。这是{{userName}}的朋友圈。请根据朋友圈动态、通讯录名单以及人物关系，模拟极其真实、生动、符合各自人设的朋友圈互动（点赞与评论）。每次调用只生成由“当前用户行为”直接触发的本轮新增互动，不负责重建或复述完整评论区。
...
3. 可见性是不可突破的权限边界：只能使用“朋友圈可见范围/可用通讯录好友”中列出的名字生成点赞、评论或楼中楼回复。未列出的人等同于完全不知道这条动态，禁止以任何称呼、别名、代称让其出现。
4. 在可用好友不为空时应生成自然互动；评论回复请保持不超过10条。可用好友为“无”时必须返回空的互动标签，不得虚构联系人。

【增量输出硬约束】
1. “当前已有点赞”和“当前已有评论”仅用于理解上下文，是只读历史记录，绝对不能作为本次输出内容重新列出。
2. 只输出由“当前用户行为”新触发的点赞、评论或楼中楼回复。禁止复制、复述、改写、概括、续列或重新编排任何已有互动，即使原句看起来仍然适合当前情境也不允许输出。
3. 已经发言过的好友可以继续产生新的回复，但必须明确承接“当前用户行为”，并说出与历史评论不同的新内容；不得用近义句重复此前表达。
4. 已经存在于“当前已有点赞”中的好友不得再次输出为点赞者，只能输出本轮新增加的点赞者。
5. 不要补全评论区，不要为了让对话看起来完整而重放前文。输出中的每一行都必须代表数据库中尚不存在、应在本轮新增的一条互动。
6. 如果当前行为没有自然产生任何新增互动，必须返回空标签：<朋友圈互动></朋友圈互动>。
...
- 直接评论使用“名字：内容”。
- 回复某人使用“名字 回复 名字：内容”。
- 回复对象必须已经出现在“当前已有评论”中，或出现在本次输出更早的一条直接评论中；没有可见上文时必须改为直接评论，禁止生成悬空回复。
- 下方仅为格式示意，不代表需要输出完整互动链；实际输出只能包含本轮新增内容。
...
⚠️ 【回复规则】：
1. 使用<wechat>标签包裹回复
2. 用 ---{{groupName}}--- 标记当前群聊窗口（必须完全匹配群名），并在下一行写 type:group
3. 每条消息一行，格式：发送者: 消息内容
4. 发送者名字必须从【群成员白名单】中选择，角色名称必须一致，严禁在群内调用不属于该群成员的其他角色！
5. 可多个群成员在群内发言，或者群成员之间互相回复，开启话题互动。
6. 禁止旁白、动作描写、心理活动
7. 红包/转账处理：群聊中必须区分定向单独红包、拼手气群红包和均分群红包。{{user}}发送的群红包会在聊天记录中显示完整类型、总金额、红包个数和状态。群成员也可以主动发送群红包，但必须单独输出完整标签：群友名: [群红包｜类型：拼手气红包｜总金额：¥xx.xx｜红包个数：x] 或 群友名: [群红包｜类型：均分红包｜总金额：¥xx.xx｜红包个数：x]，禁止只输出缺少金额和个数的 [群红包]。定向红包只显示 [给群成员名的红包 ¥xx.xx]（状态：未领取），被指定成员领取时输出 群友名: [领取红包]。拼手气红包由非发送者群成员分别输出 群友名: [领取拼手气红包¥xx.xx]，均分红包输出 群友名: [领取均分红包¥xx.xx]；AI 群成员发送者不能领取自己的红包，但{{user}}本人可以点击领取自己发送的拼手气或均分群红包。每名成员只能领取一次，领取人数不得超过红包个数，所有领取金额总和不得超过红包总额，最后一位领取者获得剩余余额。{{user}}本人通过点击群红包领取（包括自己发送的群红包），系统自动结算金额，禁止替{{user}}输出领取标签。红包被退回时输出 群友名: [退回红包]。当{{user}}在群里发起转账，收转账输出 群友名: [收款]，退回则输出 群友名: [退回转账]。所有红包和资金动作标签必须单独成行，不要和普通聊天混在同一条里。
...
群友B: 「引用 群友A: 晚上好啊」晚上好！
群友B: [转账](金额：xx元)
群友C: [给群友D的红包 ¥20.00]（状态：未领取）
群友D: [领取红包]
群友B: [群红包｜类型：拼手气红包｜总金额：¥10.00｜红包个数：4]
群友C: [领取拼手气红包¥2.80]
群友D: [群红包｜类型：均分红包｜总金额：¥10.00｜红包个数：3]
群友A: [领取均分红包¥3.33]
群友B: [收款]/[退回转账]/[退回红包]
群友D: [语音条]（语音转化出的文字内容）
...
{{dateExample}}14:30：具体事情
            ...
            wangxiang: {
                tasks: {
                    enabled: true,
                    name: '任务大厅默认提示词',
                    description: '万象任务大厅生成任务的提示词',
                    content: `【当前手机时间】
{{storyTime}}

<任务>
任务标题：xxxx
发布时间：yyyy年m月d日 hh:mm
任务开始时间：yyyy年m月d日 hh:mm
预估耗时：x小时或x天
发布者：xxx
发布者组织：xxx
发布者信誉：xxx
...
【任务生成规则】
1.仅根据故事背景生成5-10个任务.
2.生成的任务必须使用<任务></任务>进行包裹，每个任务之间用---进行分割.
3.任务不设置等级、难度或推荐等级，任务标题前不要添加等级前缀。
4.每个任务必须完整输出模板中的全部字段，不得省略字段或改变字段名。
5.发布时间和任务开始时间必须使用完整、具体的剧情日期与时间，统一使用“yyyy年m月d日 hh:mm”格式，不得使用“稍后”“今晚”“明天”等模糊表述；任务开始时间不得早于发布时间。预估耗时必须根据任务内容填写合理时长，使用“x小时”或“x天”格式，不得填写截止日期。发布者必须优先填写【角色卡信息】或故事背景中与任务相关的明确角色姓名；只有找不到可用的角色姓名时，才填写发布任务的组织、机构或组织联系人名称。不得在已有明确姓名或组织时使用“万象任务中心”等泛称。
6.任务内容应包含背景、风险和执行要求；任务目标生成2-4项，统一使用“目标名称｜0/总数量”格式，新任务的当前进度必须为0。
7.奖励和声望值必须只显示数字，无需带单位；额外奖励可以是物品名称、数量或“无”。
8.每个任务必须生成2-5条符合世界背景的任务讨论，严格使用“昵称｜时间｜讨论内容”格式，不得省略或留空。
9.必须参考【当前手机时间】生成合理的发布时间和具体任务开始时间，并根据任务目标、地点、风险和执行要求估算合理耗时；任务描述中也要明确与开始时间有关的集合、到场或执行安排。`,
                    order: 1
                },
                marketplace: {
                    enabled: true,
                    name: '商品商场默认提示词',
                    description: '万象商品商场生成商品的提示词',
                    content: `请结合角色卡、当前剧情与所选世界书，为虚拟商场生成符合世界观的商品。

【商品分类名称】
{{marketCategories}}

【生成要求】
1. 商品必须符合当前故事背景、科技或力量体系，不得生成明显脱离世界观的物品。
2.根据提供的商品分类标题，每个分类都必须独立随机生成4-10个商品，各分类的商品数量应尽量不同，禁止固定每类只生成2个。只能使用上面提供的商品分类名称，每个商品必须合理填写名称、简介、价格、库存和预计配送时间。
3. 商品名称应简洁，简介若为功能性产品应明确说明实际用途或效果，若装饰性可简单说明外观。
4. 价格与库存需结合商品价值、稀有度和当前世界的经济水平合理设置。
5.可输出商品标签，每个标签最多两字且最多两组，如医疗I急救.
6.预计配送时间必须结合商品来源、距离、体积和世界观合理填写，只能使用“x分钟”“x小时”或“x天”格式，不得填写具体到达日期。
7.下方回复格式中的物品条目只用于展示字段结构，不代表每类商品数量；实际回复必须在每个分类下按相同结构连续输出4-10个物品。
...
拟真短信:除正常短信外,可随机生成广告推销(含NSFW)、银行动账、垃圾短信、天气预报等真实拟真的短信通知.
短信来源:所有短信必须有明确的来源命名(如:具体的银行名称、账号尾号、联系人名称、匿名短信或广告推送等).
发送时间:必须填写完整年月日和时分，严格使用“YYYY年MM月DD日 HH:mm”格式，日期与当前手机时间一致.
多条短信：多条短信可使用---分割，包裹在同一个<短信></ 短信>内。`,
                    order: 13
                }
            },
...
4. 格式规范：≥1w 使用“w”，1k-9.9k 使用“k”。最多保留1位小数，换算结果必须与实际总数完全一致。

【微博配图通用格式】

① 当公开图片画面里不含②、③、④的情况下可使用通用配图格式：[图片]（中文图片描述）（English tags）。[图片]（两个女生穿着便装并肩走在街边，背景是虚化的城市灯光）（2girls, adult character, street snapshot, casual outfits, walking side by side, blurred city background, anime illustration）。
...
以上①-④配图格式，第一个括号写中文图片描述供微博卡片背面展示，第二个括号只能写英文逗号分隔的英文生图 tag，不要写中文、解释或完整句子；少写第二个括号会被系统判定为格式错误，图片不会生成。

请严格遵守以下<Weibo>标签输出完整内容，生成的微博严禁替代user发送微博内容。按照以下格式输出每一条微博，使用---隔开，严禁回复任何非标签内的内容或篡改格式。
完整格式如下：
<Weibo>
用户粉丝数：[变化后的粉丝总数；无变化时填写当前值]
微博热搜：
1.[与剧情相关的衍生热搜1] （爆）
2.[与剧情相关的衍生热搜2] （热）
3.[用户查询的目标热搜词] （新）
...
来自：[手机型号，如：iPhone 15 Pro / 微博网页版]
正文：[微博正文内容，需符合账号人设，带适当的#话题#和@提及]
配图：[图片]（中文图片描述）（English tags）[图片]（中文图片描述）（English tags）（注：配图为可选，如果该微博是纯文字，请直接省略“配图：”这一整行）
数据：转发 [数字] | 评论[数字，必须等于下方实际评论数] | 点赞 [数字]
评论区：
1. [网友昵称1] (ip[市区])：内容1
2. [网友昵称2] (ip[市区])：内容2
...
5. 需要体现地域IP属性
6. 仅当本轮互动有明确的公开关注/取关依据时，才可额外返回变化后的 followers 总数（正整数）；无变化、无法判断或没有依据时必须省略，禁止返回0或增减量
...
4. 必须带上地域IP属性。
5. 仅当本轮互动有明确的公开关注/取关依据时，才可额外返回变化后的 followers 总数（正整数）；无变化、无法判断或没有依据时必须省略，禁止返回0或增减量。
...
- 严禁替用户决定是否同意好友申请或联播申请；你只负责产出待处理申请。`,
            ...
            games: {
                undercoverDealer: {
                    enabled: true,
                    name: '谁是卧底开局生成',
                    description: '生成 AI 玩家资料与本局身份词',
                    content: this._getDefaultUndercoverDealerPromptTemplate(),
                    order: 1
                },
                undercoverSpeech: {
                    enabled: true,
                    name: '谁是卧底 AI 发言',
                    description: '控制每名 AI 玩家在轮到自己时的发言',
                    content: this._getDefaultUndercoverSpeechPrompt(),
                    order: 2
                }
            },
